...
#define FLAGS0	DT_GPIO_FLAGS(LED2_NODE, gpios)
...
void rx_callback_function(struct zcan_frame *frame, void *arg){
    ...
    ourLED = device_get_binding(LED2);
    gpio_pin_configure(ourLED, PIN2, GPIO_OUTPUT_ACTIVE | FLAGS0);
...
int main(){
   ...
   const struct zcan_filter activity1_filter = {
    ...
    //activity3
    filter_id = can_attach_workq(can_dev, &k_sys_work_q, &rx_work, rx_callback_function, NULL, &activity1_filter);
